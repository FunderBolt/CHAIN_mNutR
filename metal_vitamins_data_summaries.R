...
# load metal raw data
metal_data <- read_csv("metal_data.csv", 
                       na = "ND") # ND (not done) equivalent to NA

# change to long format
metal_data_long <- metal_data %>% 
  pivot_longer(cols = -`Sample Id`, 
               names_to = "test_name", 
               values_to = "results") %>% 
  select(record_id = `Sample Id`, everything()) %>% 
  mutate(
    record_id = str_sub(record_id, 9, 16)
  )

# left join for site variable and ABC groub
metal_data_long <- metal_data_long %>% 
  left_join(chain_data %>% mutate_at(vars(record_id), as.character) %>% 
              select(record_id, site, categ_enrol), by = "record_id")


# final clean data
write.csv(metal_data_long, "metal_data_long.csv", row.names = F)


# How many ABC groups in the datase
with(
  metal_data_long, 
  table(categ_enrol) # equal proportion of community and SAM
)
...
test_names <- unique(metal_data_long$test_name)

for (testname in test_names){
  
  plot_lst[[testname]] <- metal_data_long %>% 
...
for (testname in test_names){
  ...
  box_plots[[testname]] <- metal_data_long %>% 
    filter(test_name == testname) %>% 
