...
list_packages <- c("here","mgsub","tidyverse","FactoMineR","factoextra","mixOmics",#"ggROC","pROC",
                   "ggforce", "directlabels","ggpubr")
...
### making groups human readable 
levels(data$Group)<-c("1" , "2" , "3" , "Control", "Low protein", "Rapamycin" )
data$Group<- mgsub(data$Group,c('1','2','3'), c("Control", "Low protein", "Rapamycin"))
data$Group <-droplevels(data$Group)


head(data)
X<-as.matrix(data[,c(7:ncol(data))])
#X<-as.matrix(data[!data$Group=="Control", c(7:ncol(data))])
X
...
#set outcome
outcome<-data$Group
#outcome<-data$Group[!data$Group=="Control"]
outcome
...
summary(perf.out)
 ...
 #colnames(idata)[1:20]
 #out.splsDA<-mixOmics::splsda(X,outcome, keepX=c(10,10), ncomp=2, max.iter = 500, tol = 1e-06)
 #out.splsDA$explained_variance
 ...
 plotVar(out.splsDA,cex=3, cutoff=0.50)
 ...
 write.csv(plsda_features,file=paste0("mTOR_MAK_plsDA&splsDA_features_Comp1&2_noProtCorr_cut5_", Sys.Date() , ".csv"))
...
### plotting vars with 0.5 cut off
...
metabs<-metabs[ abs(metabs$sPLS_Loading_comp1) > 0.1 | abs(metabs$sPLS_Loading_comp2) > 0.1,  ]

write.csv(metabs,file=paste0("mTOR_MAK_splsDA_featuresSELECTED_Comp1&2_noProtCorr_cut5_load.1_", Sys.Date() , ".csv"))
...
# plot_Data$names<-c("UDP", "GDP", "cyclic.AMP", "Acetyl Phosphate", 
...
# 
# plot_Data$names<-c("UDP", "cyclic.ADP.ribose", "cyclic.AMP", "Acetyl.CoA", "Glycerylaldehyde.3P", 
#                    "Mannose.6P", "Glycolic.acid", "Isocitric.acid")
...
# plot_Data$names<-c("GDP", "Acetyl.Phosphate", "X2.3.bisP.Glycerate", "total.of.Fructose.bisP.Glucose.1.6.bisP", 
#                    "Acetyl.CoA", "NAD.", "NADH", "NADPH", "Glucose", "Glycerylaldehyde.3P", 
#                    "Ribose.5P", "a.Hydroxyglutaric.acid", "Glycolic.acid", "Isocitric.acid", 
#                    "Pyruvic.acid")
...
p1<-ggplot(aes(x=x, y=y), data=plot_Data)+
 ...
 scale_x_continuous(lim=c(-1.5,1.5))+
  ...
  geom_dl(aes(x = x, y = y, label = names), 
          method = list(dl.trans(x = x - 0.2, y = y + 0), "first.bumpup", cex = 0.9)) +
  labs(x = "Component-1", y = "Component-2")+
  theme_bw()+
  theme(legend.position = "none", axis.title = element_text(size = 14), axis.text=element_text(color = "grey30", size=14))
p1
...
p2<-ggplot(aes(x=x, y=y), data=vars.in)+
  geom_point(aes(col=group, fill=group), shape=c(21), size=5)+
  ...
  labs(x = paste0("Variate-1 (", 100*round(out.splsDA$explained_variance$X[1],3) , "% explained variance)"), y = paste0("Variate-2 (", 100*round(out.splsDA$explained_variance$X[2],3) , "% explained variance)"))+
  theme_bw()+
  theme(axis.title = element_text(size = 14), axis.text=element_text(color = "grey30", size=14), legend.position = "none")+
  ...
  stat_mean(aes(group = group),size=4, shape=8, color= c("black","black","grey30"), na.rm = TRUE, show.legend = NA,
            inherit.aes = TRUE)
p2


p_all<-ggarrange(p2,p1, ncol = 2, nrow =1, common.legend = FALSE) 
p_all

# saves as .svg size 1000 800
ggsave(file="Fig_PLSDA_byGroups_2020-11-07.svg", plot=p_all, width=16, height=8)
