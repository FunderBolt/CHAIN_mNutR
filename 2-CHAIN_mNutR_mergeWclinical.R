...
# load met data
met_data <- read.csv(paste0(dirname(dirname(dirname(here("6-Data")))), "/5-Data","/CHAIN_mNutR_metabolite_data_2020-11-06.csv"), row.names = 1) # ND (not done) equivalent to NA
...
#adm_blood<- read.csv("C:\\Users\\cbour\\OneDrive\\Desktop\\2019_DataCuration\\CHAIN_Data\\CHAIN_Data_v9\\laboratory\\Biochemistry\\clean_adm_clinical_chem_data.csv")
...
fulldata<-merge(demograph, anthro, by="record_id", all=TRUE)
fulldata<-merge(fulldata, outcome, by="record_id", all=TRUE)
fulldata<-merge(fulldata, met_data, by="record_id", all.y=TRUE)

dput(colnames(fulldata))
# final clean data
write.csv(fulldata, file=paste0("CHAIN_mNutR_fulldata_",Sys.Date(),".csv"))
